...
class McategoryController extends Controller
{
    ...
    {
        $mcats = MainCategory::where('status', 1)
            ->orderBy('main_mcat_position', 'asc')   // optional ordering
            ->get(['main_mcat_id','main_mcat_name']); // only the fields your front‑end needs
    ...
    public function mainCatToggleStatus(Request $request, $id)
    {
    ...
    public function mcatToggleStatus(Request $request, $id)
    {
        $cat = Mcategory::findOrFail($id);
        $cat->status = $request->status;
        $cat->save();
    ...
    public function msubcatToggleStatus(Request $request, $id)
    {
        $cat = Msubcategory::findOrFail($id);
        $cat->status = $request->status;
        $cat->save();
