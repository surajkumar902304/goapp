...
class BannerController extends Controller
{
    ...
    public function loyaltyRewardVlist()
    {
        $loyaltyBanners = LoyaltyRewardBanner::get();

        return response()->json([
        'status'       => true,
        'loyalty_rewards' => $loyaltyBanners
        ], 200);
    }

    public function addLoyaltyReward(Request $request)
    {
        $request->validate([
            'loyalty_reward_banner_image' => 'nullable|image|max:2048',
        ]);

        $banner_imgpath = null;
        if ($request->hasFile('loyalty_reward_banner_image')) {
            $image  = $request->file('loyalty_reward_banner_image');
            $filename = 'loyalty_reward_' . uniqid() . '.png';
            $img = Image::make($image->getRealPath());
            $banner_imgpath      = 'goapp/images/loyalty_reward/' . $filename;
            Storage::disk('s3')->put($banner_imgpath, (string) $img->encode());
        }

        $loyalty_reward  = new LoyaltyRewardBanner();
        $loyalty_reward->loyalty_reward_banner_image   = $banner_imgpath;
        $loyalty_reward->save();

        return response()->json(['status' => true]);
    }

    public function editLoyaltyReward(Request $request)
    {
        $request->validate([
            'loyalty_reward_banner_id'    => 'required|exists:loyalty_reward_banners,loyalty_reward_banner_id',
            'loyalty_reward_banner_image' => 'nullable|image|max:2048',
        ]);

        $loyaltybanner = LoyaltyRewardBanner::find($request->loyalty_reward_banner_id);
        $banner_imgpath = $loyaltybanner->loyalty_reward_banner_image;
        ...
        if ($request->hasFile('loyalty_reward_banner_image')) {
            ...
            $loyaltybanner->loyalty_reward_banner_image = $banner_imgpath;
        }
