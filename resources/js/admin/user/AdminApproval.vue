  ...
  <div class="page-margin-20-40">
    ...
    <v-row class="mt-0 pt-0">
      <v-col cols="12" class="mt-2">
        <v-card elevation="5" style="background-color: transparent;">
          ...
          <v-data-table dense v-model="selected" show-select item-key="id" :items="filteredUsers" :headers="userHeaders"
            :search="ssearch"
            ...
            <template v-slot:top>
              <v-text-field v-model="ssearch" class="px-2 py-1" clearable dense outlined hide-details
                prepend-inner-icon="mdi-magnify mb-2" placeholder="Search Customers" />
            </template>
            ...
            <template #header.rep_code>
              <div class="text-center">Rep&nbsp;code</div>
            </template>
            ...
            <template #item.action1="{ item }">
              <div class="text-center">
                <v-chip v-if="item.admin_approval === 'Pending' || item.admin_approval === 'Declined'" color="green"
                  outlined pill small style="cursor:pointer" @click="changeStatus(item, 'Approved')">Approve</v-chip>
              </div>
            </template>
            ...
            <template #item.action2="{ item }">
              <div class="text-center">
                <v-chip v-if="item.admin_approval === 'Pending' || item.admin_approval === 'Approved'" color="red" outlined
                  pill small style="cursor:pointer" @click="changeStatus(item, 'Declined')">Decline</v-chip>
              </div>
            </template>
            <template #header.action3>
              <div class="text-center">Action</div>
            </template>
            <template #item.action3="{ item }">
              <div class="text-center">
                <v-chip color="primary" class="white--text" outlined pill small style="cursor:pointer"
                  @click="editItem(item)">
      ...
      <v-card elevation="5">
        ...
        <v-form v-model="fsvalid" @submit.prevent="updateUser">
          <v-card-text>
            <v-container>
              <v-row>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.name" label="Name"
                    :rules="[rules.required, rules.max255]" required /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.email" label="Email"
                    :rules="[rules.required, rules.email, rules.max255]"
                    :error-messages="emailError ? [emailError] : []" @blur="checkEmailUnique" required /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.mobile" label="Mobile"
                    :rules="[rules.required, rules.mobile, rules.max20]" required type="tel" inputmode="tel" /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.company_name" label="Company Name"
                    :rules="[rules.required, rules.max255]" required /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.address1" label="Address 1"
                    :rules="[rules.required, rules.max255]" required /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.address2" label="Address 2"
                    :rules="[rules.max255]" /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.city" label="City"
                    :rules="[rules.required, rules.max100]" required /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.country" label="Country"
                    :rules="[rules.required, rules.max100]" required /></v-col>
                <v-col cols="12" sm="6"><v-text-field v-model="selectedUser.postcode" label="Postcode"
                    :rules="[rules.required, rules.max20]" required /></v-col>
              </v-row>
            </v-container>
          </v-card-text>
          <v-card-actions>
            ...
            <v-btn class="btn-32-text-12" type="submit"
              ...
              :loading="submitting" :disabled="!fsvalid || submitting || !emailUnique">Update</v-btn>
          </v-card-actions>
        </v-form>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="tagDialog" max-width="400px">
      <v-card elevation="5">
        ...
        <v-card-text>
          <v-select v-model="selectedTagId" :items="userTags" item-value="user_tag_id" item-text="user_tag_name"
            label="Select Tag" dense outlined />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
    ...
    <v-dialog v-model="repDialog" max-width="400px">
      <v-card elevation="5">
        ...
        <v-card-text>
          <v-select v-model="selectedRepId" :items="reps" item-value="rep_id" item-text="rep_code"
            label="Select Rep Code" dense outlined />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
      ...
      <v-card elevation="5">
        ...
        <v-form v-model="walletFormValid" @submit.prevent="submitWalletUpdate">
          <v-card-text>
            ...
            <v-text-field v-model="walletForm.balancevalue"
              :label="walletForm.balancekey === 'add' ? 'Amount to Add' : 'Amount to Remove'" type="number" step="0.01"
              :rules="[
                v => v === '' || parseFloat(v) >= 0 || 'Only positive numbers allowed',
                v => v === '' || /^\d+(\.\d{1,2})?$/.test(v) || 'Up to 2 decimal places allowed'
              ]" />
            <v-text-field v-model="walletForm.reference" dense placeholder="Enter Reference" label="Reference"
              :rules="[v => !v || v.length <= 255 || 'Max 255 chars']"></v-text-field>
          </v-card-text>

          <v-card-actions>
            <v-spacer />
            <v-btn class="btn-32-text-12" type="submit" color="success" :disabled="!walletFormValid || loading"
              :loading="loading">
              Update
            </v-btn>
          </v-card-actions>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        max100: v => !v || String(v).length <= 100 || 'Max 100 characters',
        max20: v => !v || String(v).length <= 20 || 'Max 20 characters',
        email: v => {
          if (!v) return true; 
          const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
          return re.test(String(v).toLowerCase()) || 'Enter a valid email';
        },
        mobile: v => {
          if (!v) return true;
        ...
        postcode: v => {
          if (!v) return true;
          return /^[A-Za-z0-9 -]{3,10}$/.test(String(v)) || 'Enter a valid postcode';
        },
      },
