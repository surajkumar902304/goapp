                    ...
                    <v-card v-if="vitems.length > 0 && vitems.some(item => item.optname && item.optname.length > 0)" class="mt-3 pa-3">
                        ...
                        <v-data-table :items="vitems" :headers="variantHeaders" class="elevation-1">
                            <template v-slot:item.variantImage="{ item, index }">
                                <v-card class="d-flex align-center justify-center" height="50" outlined @click="triggerVariantFileInput(index)">
                            ...
                            <template v-slot:item.price="{ item }">
                                <v-text-field v-model="item.price" small dense outlined />
                            </template>
                            <template v-slot:item.stock="{ item }">
                                <v-text-field v-model="item.stock" small dense outlined type="number" />
                            </template>
                            <template v-slot:item.sku="{ item }">
                                <v-text-field v-model="item.sku" small dense outlined />
                            </template>
                            <template v-slot:item.barcode="{ item }">
                                <v-text-field v-model="item.barcode" small dense outlined />
                            </template>
                        </v-data-table>
                    </v-card>
                </v-col>
                        ...
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    ...
                                    <v-checkbox v-model="pro.selectedSalesChannel" label="Online Store" value="Online Store" @change="handleCheckboxChange('online ')" />
