        ...
        <v-form @submit.prevent="saveProductData" v-model="fvalid">
            ...
            <v-dialog v-model="unsavedDialog" max-width="400">
                <v-card elevation="5">
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn text class="btn-32-text-12" color="primary" @click="saveProductData">Save</v-btn>
                        <v-btn text class="btn-32-text-12" color="red" @click="exitWithoutSaving">Back Without Saving</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
            ...
            <v-row>
                <v-col cols="12" md="9">
                    ...
                    <v-card v-if="!Object.keys(variants).length > 0" class="mt-3" elevation="5">
                            ...
                            <v-row>
                                <v-col cols="12" md="4">
                                    ...
                                    <v-row dense>
                                        ...
                                        <v-col cols="5">
                                            <v-select v-model="weightUnit" :items="['kg', 'g']" dense label="Unit"></v-select>
                                        </v-col>
                                    </v-row>
                                </v-col>
                            </v-row>
                                    ...
                                    <v-sheet class="pa-2 rounded border mb-3" style="background-color: rgb(249, 249, 249);">
                                        <v-row class="align-center">
                                            ...
                                            <v-col cols="4" class="d-flex justify-end">
                                                <v-btn icon class="btn-32-text-12" color="primary">
                                                    <v-icon @click="editVariant(key)">mdi-pencil</v-icon>
                                                </v-btn>
                                                <v-btn icon class="btn-32-text-12" color="error">
                    ...
                    <v-card v-if="Object.keys(variants).length > 0" class="mt-3 pa-3" elevation="5">
                        ...
                        <v-data-table :items="variantItems" :headers="variantHeaders" class="elevation-1">
                            ...
                            <template v-slot:item.price="{ item }">
                                <v-text-field v-model="item.price" small dense outlined type="number" hide-details></v-text-field>
                            </template>
                            <template v-slot:item.compareprice="{ item }">
                                <v-text-field v-model="item.compareprice" small dense outlined type="number" hide-details></v-text-field>
                            </template>
                            <template   v-slot:item.stock="{ item }">
                                <v-text-field class="stock_keeper"  v-model="item.stock" small dense outlined type="number":rules="avstockRules"></v-text-field>
                            ...
                            <v-row>
                                <v-col cols="12" md="12">
                                    ...
                                    <!-- <v-autocomplete v-model="pro.brand" :items="mbrands" item-text="mbrand_name" item-value="mbrand_id" label="Brand" 
                                        ...
                                        <template v-slot:no-data>
                                            <v-btn class="btn-32-text-12" @click="addNewBrand" :disabled="!typedBrand?.trim()" >
                                                Add "{{ typedBrand }}"
                                            </v-btn>
                                        </template>
                                    </v-autocomplete> -->
...
export default {
    ...
    computed: {
        ...
        variantItems() {
            ...
            let vcompareprice = this.compareprice;
