    ...
    <div style="margin: 20px 60px !important;">
        ...
        <v-form @submit.prevent="updateProductData" v-model="fvalid">
            ...
            <v-row>
                <v-col cols="12" md="9">
                    <v-card elevation="5">
                        <v-card-text>
                            <v-row>
                                ...
                                <v-col cols="12" md="6" class="desc-text-area">
                    ...
                    <v-card v-if="!Object.keys(variants).length" class="mt-3" elevation="5">
                            ...
                            <v-row>
                                ...
                                <v-col cols="12" md="4" class="py-0 d-flex align-center">
                                    <div class="d-flex align-center" style="margin-top: 12px;">
                                        <v-checkbox v-model="taxable" hide-details class="pa-0" style="margin: 0; padding-right: 4px;" density="compact" ></v-checkbox>
                                        <span style="font-size: 13px; margin-top: 1px;">Charge tax on this product</span>
                                    </div>
                                </v-col>
                            </v-row>
                        </v-card-text>
                        ...
                        <v-card-text>
                            <v-row>
                                <v-col cols="12" md="4">
                                    ...
                                    <v-row dense>
                                        <v-col cols="7">
                                            <v-text-field v-model.number="weight" type="number" dense label="Weight" :rules="weightRules"></v-text-field>
                                        </v-col>
                                        <v-col cols="5">
                                            <v-select v-model="weightUnit" :items="['kg', 'g']" dense label="Unit" hide-details></v-select>
                                        </v-col>
                                    </v-row>
                                </v-col>
                            </v-row>
                                    ...
                                    <v-sheet class="pa-2 rounded border mb-3" style="background-color: #f9f9f9;">
                                        <v-row class="align-center">
                                            ...
                                            <v-col cols="4" class="d-flex justify-end">
                                                <v-btn class="btn-32-text-12" icon color="primary"><v-icon @click="editVariant(key)">mdi-pencil</v-icon></v-btn>
                                                <v-btn class="btn-32-text-12" icon color="error"><v-icon @click="removeOption(key)">mdi-delete</v-icon></v-btn>
                                            </v-col>
                                        </v-row>
                                    </v-sheet>
                                </div>
                    ...
                    <v-card v-if="vitems.length > 0 && vitems.some(item => item.optname && item.optname.length > 0)" class="mt-3 pa-3" elevation="5">
                        ...
                        <v-data-table :items="vitems" :headers="variantHeaders" class="elevation-1">
                            ...
                            <template v-slot:item.price="{ item }">
                                <v-text-field v-model="item.price" small dense outlined type="number" hide-details />
                            </template>
                            <template v-slot:item.compareprice="{ item }">
                                <v-text-field v-model="item.compareprice" small dense outlined type="number" hide-details />
                            </template>
                            <template v-slot:item.stock="{ item }">
                                <v-text-field class="stock_keeper" v-model="item.stock" small dense outlined type="number":rules="avstockRules"  />
                            </template>
                ...
                <v-col cols="12" md="3" elevation="5">
                    <v-card>
                            ...
                            <v-row>
                                <v-col cols="12" md="12">
                                    ...
                                    <div class="d-flex align-center" style="margin-top: 8px;">
                                        <v-checkbox v-model="pro.selectedSalesChannel" value="Online Store" hide-details class="pa-0" style="margin: 0; 
                                    ...
                                    <div class="d-flex align-center" style="margin-top: 4px;">
                                        <v-checkbox v-model="pro.selectedSalesChannel" value="Other" hide-details class="pa-0" style="margin: 0; padding-right: 4px;" 
                            ...
                            <v-row>
                                <v-col cols="12" md="12">
                                    ...
                                    <!-- <v-autocomplete v-model="pro.brand" :items="mbrands" item-text="mbrand_name" item-value="mbrand_id" label="Brand" 
                                        ...
                                        <template v-slot:no-data>
                                            <v-btn class="btn-32-text-12" @click="addNewBrand" :disabled="!typedBrand" >
                                                Add "{{ typedBrand }}"
                                            </v-btn>
                                        </template>
                                    </v-autocomplete> -->
        ...
        <v-dialog v-model="duplicateDialog" max-width="400">
            <v-card elevation="5">
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn class="btn-32-text-12" text color="grey" @click="duplicateDialog = false">Cancel</v-btn>
                    <v-btn class="btn-32-text-12" text color="red" @click="performDuplicate">Duplicate</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            variantHeaders: [
                { text: "Image", value: "variantImage", editable: true, sortable: false },
                { text: "Variant", value: "variant", sortable: false },
                { text: "Price", value: "price", editable: true, sortable: false },
            ...
            stockRules: [
             ...
             avstockRules: [
                (v) => v === null || v === '' || /^\d+$/.test(v) || "Enter positive number",
                (v) => v === null || v === '' || v >= 0 || "Enter positive number",
            ],

            weightRules: [
            v => v >= 0 || "Weight must be greater than 0",
            v => v === "" || /^\d+(\.\d{1,2})?$/.test(v) || "Weight up to 2 decimal places"
            ]
        };
    },
    mounted() {
    ...
    methods: {
        ...
        generateVariantCombinations() {
            ...
            if (foundOld) {
                ...
                return {
                    ...
                    compareprice: foundOld.compareprice,
        ...
        async updateProductData() {
            ...
            if (this.vitems.length > 0) {
                ...
                pdata.append(`variants[${index}][compareprice]`, variant.compareprice ?? "");
                ...
                const capitalizedOptValue = Object.fromEntries(
                        Object.entries(variant.optvalue ?? {}).map(([key, value]) => [
                            key,
                            value
                            .split(' ')
                ...
                pdata.append(`variants[${index}][optvalue]`, JSON.stringify(capitalizedOptValue));
...
.stock_keeper .v-text-field__details{
    position: absolute;
    bottom: -18px;
}
.tag-text-area label {
