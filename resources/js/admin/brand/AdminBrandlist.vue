<template>
    <div class="page-margin-20-40">
        <v-container fluid class="pt-0">
            <v-row class="mt-0 pt-0">
                ...
                <v-col cols="12" md="1" class="p-0 ps-2 text-end">
                    <v-btn small class="text-none w-100 btn-32-text-12"
        ...
        <v-row class="mt-0">
            <v-col cols="12">
                <v-card elevation="5">
                    <v-data-table v-model="selected" item-key="mbrand_id" :show-select="true" :items="mbrands"
                        :headers="mbrandsHeaders" :search="ssearch" :footer-props="{
                        ...
                        <template v-slot:top>
                            <v-row dense class="mx-1 pb-1">
                                <v-text-field v-model="ssearch" class="m-2" clearable dense outlined hide-details
                                    prepend-inner-icon="mdi-magnify mb-2" placeholder="Search Brands" />
                            </v-row>
                        </template>
                        <template v-slot:item.mbrand_image="{ item }">
                            <v-img :src="item.mbrand_image ? cdn + item.mbrand_image : ''" cover width="50" height="50" class="ma-1" style="border: 1px solid #e0e0e0; border-radius: 10px;">
                    ...
                    <v-card-text>
                        ...
                        <div class="d-flex flex-column align-center">
                            <v-card-actions class="pb-0 pt-0">
                                <span class="body-2 fw-semibold">{{ isImageSelected ? 'Selected Image' : 'Select Image'
                                    }}</span>
                            </v-card-actions>
                            <input ref="imageInput" type="file" accept="image/*" style="display:none"
                                @change="handleImageUpload" />
                            <div class="uploader-box mb-2" @click="triggerFileInput">
                                <v-img v-if="isImageSelected" :src="imagePreview" class="rounded" max-width="150"
