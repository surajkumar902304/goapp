<template>
  <div class="page-margin-20-40">
    <v-container fluid class="pt-0">
      <v-row class="mt-0 pt-0">
        <v-col cols="12" md="11" class="p-0">
          <h2 class="text-h6 mb-1">Top Seller Sliders</h2>
        </v-col>
    ...
    <v-card elevation="5" class="mt-4">
      <v-data-table v-model="mainSel" :items="sliderRows" :headers="sliderHdr" :search="mainSearch"
        item-key="slider_row_id" show-select :footer-props="{ 'items-per-page-options': [10, 25, 50] }">
        <template v-slot:top>
          <v-row dense class="mx-1 pb-1">
            <v-text-field v-model="mainSearch" class="m-2" clearable dense outlined hide-details
              prepend-inner-icon="mdi-magnify mb-2" placeholder="Search Top Seller Sliders" />
          </v-row>
        </template>
    ...
    <v-dialog v-model="modalOpen" max-width="750">
      <v-card elevation="5">
        ...
        <v-card-text>
          <v-text-field dense outlined prepend-inner-icon="mdi-magnify" v-model="modalSearch"
            placeholder="Search Top Seller Sliders" />
          <v-data-table v-model="modalSel" :items="allVariants" :headers="modalHdr" :search="modalSearch"
            item-key="mvariant_id" show-select return-object :footer-props="{ 'items-per-page-options': [10, 25, 50] }">
            <template v-slot:item.img="{ item }">
              <v-img :src="item.img ? cdn + item.img : ''" cover width="50" height="50" class="ma-1" style="border: 1px solid #e0e0e0; border-radius: 10px;">
                <template #placeholder>
                  <div class="d-flex align-center justify-center fill-height">
      ...
      <v-card elevation="5">
        ...
        <v-card-actions>
          ...
          <v-btn class="btn-32-text-12" text color="red" :loading="deleteLoading" :disabled="deleteLoading"
            @click="performRowDelete">Remove</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-card elevation="5">
        ...
        <v-card-actions>
          ...
          <v-btn class="btn-32-text-12" text color="red" :loading="bulkDeleteLoading" :disabled="bulkDeleteLoading"
            @click="performBulkDelete">Remove</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    async fetchRows() {
      const { data } = await axios.get('/admin/top-sellers');
    ...
    async save() {
      ...
      await axios.post('/admin/top-sellers', { variant_ids: ids });
    ...
    async performRowDelete() {
      ...
      try {
        await axios.delete(`/admin/top-sellers/${this.delTarget}`);
    ...
    async performBulkDelete() {
      ...
      try {
        ...
        await axios.post('/admin/top-sellers/bulk-delete', { ids });
