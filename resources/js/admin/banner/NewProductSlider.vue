  ...
  <div class="page-margin-20-40">
      ...
      <v-data-table v-model="mainSel" :items="sliderRows" :headers="sliderHdr" :search="mainSearch" item-key="slider_row_id" show-select
        ...
        <template v-slot:top>
          <v-row dense class="mx-1 pb-1">
            <v-text-field v-model="mainSearch" class="m-2" clearable dense outlined hide-details prepend-inner-icon="mdi-magnify mb-2" placeholder="Search New Products Sliders"/>
          </v-row>
        </template>
    ...
    <v-dialog v-model="modalOpen" max-width="750">
      <v-card elevation="5">
        ...
        <v-card-text>
          <v-text-field dense outlined prepend-inner-icon="mdi-magnify" v-model="modalSearch" placeholder="Search New Products Sliders"/>
