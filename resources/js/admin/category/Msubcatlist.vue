    ...
    <v-container fluid class="pt-0">
      <v-row class="mt-0 pt-0">
        ...
        <v-col cols="12" md="2" class="p-0 ps-2 text-end">
          <v-btn color="secondary" small :to="'/admin/sub-categories/addview'" router class="text-none btn-32-text-12" style="font-weight: bold; color: #1976d2; 
                ...
                <v-data-table v-model="selected" :show-select="true" item-key="msubcat_id" :items="subcats" :headers="msubcatsHeaders" :search="ssearch" :footer-props="{
                    ...
                    <template v-slot:top>
                        <v-row dense class="mx-1 pb-1">
                            ...
                            <v-text-field class="m-2" v-model="ssearch" dense outlined clearable hide-details prepend-inner-icon="mdi-magnify mb-2" placeholder="Search Sub-category name"/>
                            </v-col>
                            <v-col cols="6" md="2" class="pa-0 pr-3">
                                <v-autocomplete class="m-2" v-model="publishFilter" :items="[ { text: 'All', value: null }, { text: 'Online Store', value: 'Online Store' }, { text: 'Other', value: 'Other' } ]" item-text="text" item-value="value" outlined dense label="Publishing" hide-details @change="publishByStatus"/>                      
                            </v-col>
                            <v-col cols="6" md="2" class="pa-0">
                                <v-autocomplete class="m-2" v-model="typeFilter" :items="[ { text: 'All', value: null }, { text: 'Smart', value: 'smart' }, { text: 'Manual', value: 'manual' } ]" item-text="text" item-value="value" outlined dense label="Type" hide-details @change="typeByStatus"/>
                            </v-col>
                        </v-row>
                    </template>
  ...
  data () {
    return {
        ...
        msubcatsHeaders: [
            ...
            { text: 'Image', value: 'msubcat_image', sortable: false, width: '100px' },
            { text: 'Sub-category name', value: 'msubcat_name', width: '250px' },
            { text: 'Category name', value: 'mcat_name', sortable: false, width: '250px' },
            { text: 'Collection type', value: 'msubcat_type', sortable: false, width: '200px' },
            { text: 'Status', value: 'status', sortable: false, width: '100px' },
            { text: 'Actions', value: 'actions', sortable: false, width: '175px' },
