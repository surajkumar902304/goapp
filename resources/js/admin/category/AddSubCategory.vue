<template>
  <div style="margin: 20px 60px !important;" class="page-sub-cat-add">
    <v-row>
      <h2 class="text-h6">Add New Sub-Category</h2>
    </v-row>
    <v-row class="mt-0">
      <v-col cols="6" class="d-flex">
        <v-btn style="color: #1976d2; background-color: white !important; border: 1px solid #1976d2 !important;"
          :loading="backLoading" :disabled="backLoading" small class="btn-32-text-12" @click="navigateBack">
      ...
      <v-col cols="6" class="text-end">
        <v-btn style="color: #ef0b0b; background-color: white !important; border: 1px solid #ef0b0b !important;" small
          outlined @click="discard" class="mr-2 btn-32-text-12">Discard</v-btn>
        <v-btn small style="color: #1976d2; background-color: white !important; border: 1px solid #1976d2 !important;"
          :loading="saveLoading" :disabled="saveDisabled" @click="saveSubCategory" class="btn-32-text-12">
          <template v-slot:loader>
            <v-progress-circular indeterminate size="20" color="white"></v-progress-circular>
          </template>
      ...
      <v-col cols="12" md="9">
        <v-card elevation="5">
          <v-card-text>
            ...
            <v-text-field dense outlined v-model="form.subcatname" label="Sub-Category Name" :rules="msubcatnameRule"
              @blur="checkDuplicate" />
          </v-card-text>
        </v-card>
        ...
        <v-card elevation="5" class="my-3">
          ...
          <v-card-text>
            <v-radio-group v-model="mcattype" class="mt-1" hide-details>
              <div class="d-flex align-center" style="margin-bottom: 8px;">
                <v-radio value="manual" hide-details class="pa-0" style="margin: 0; padding-right: 4px;"
                  density="compact"></v-radio>
                <span style="font-size: 13px; margin-top: 1px;">Manual</span>
              </div>
              <div class="d-flex align-center">
                <v-radio value="smart" hide-details class="pa-0" style="margin: 0; padding-right: 4px;"
                  density="compact"></v-radio>
                <span style="font-size: 13px; margin-top: 1px;">Smart</span>
              </div>
            </v-radio-group>
          </v-card-text>
        </v-card>
        ...
        <v-card v-if="mcattype === 'manual'" elevation="5" class="my-3">
          <v-card-subtitle>Products</v-card-subtitle>
          <v-card-text>
            <v-row>
              <v-col cols="6">
                <v-text-field dense hide-details outlined prepend-inner-icon="mdi-magnify mb-2" v-model="productSearch"
                  placeholder="Search Product" />
              </v-col>
              <v-col cols="2">
                <v-btn style="color: #1976d2; background-color: white !important; border: 1px solid #1976d2 !important;"
                  dense outlined @click="productDialog = true" class="btn-32-text-12 mt-1">Browse</v-btn>
              </v-col>
              <v-col cols="4">
            ...
            <div v-if="selectedProducts.length" class="px-4 pb-4">
              ...
              <v-list dense>
                <v-list-item v-for="p in selectedProducts" :key="p.mproduct_id">
                  ...
                  <v-list-item-action>
                    <v-btn class="btn-32-text-12" icon
                      @click="removeProduct(p.mproduct_id)"><v-icon>mdi-close</v-icon></v-btn>
                  </v-list-item-action>
                </v-list-item>
              </v-list>
            </div>
        ...
        <v-card v-else elevation="5" class="my-3">
          <v-card-subtitle>Conditions</v-card-subtitle>
          <v-card-text>
            <div class="d-flex align-center" style="gap: 8px; font-size: 12px;">
              <span style="margin-top: 10px;">Product must match:</span>
              <v-radio-group v-model="acondition" row hide-details class="d-flex align-center ml-2"
                style="margin-bottom: 10px !important; margin-top: 10px !important;">
                <div class="d-flex align-center" style="margin-left: 8px;">
                  <v-radio value="all" hide-details class="pa-0" style="margin: 0; padding-right: 4px;"
                    density="compact"></v-radio>
                  <span style="font-size: 12px;">All conditions</span>
                </div>

                <div class="d-flex align-center" style="margin-left: 12px;">
                  <v-radio value="any" hide-details class="pa-0" style="margin: 0; padding-right: 4px;"
                    density="compact"></v-radio>
                  <span style="font-size: 12px;">Any condition</span>
                </div>
              </v-radio-group>
            </div>
            ...
            <div v-for="(row, idx) in conditions" :key="idx" class="row mb-2">
              ...
              <div class="col-md-4 d-flex">
                <div v-if="['Inventory stock'].includes(getFieldNameById(row.tag))">
                  <v-text-field dense outlined v-model="row.value" label="Value" type="number" class="flex-grow-1"
                ...
                <div v-else-if="['Type'].includes(getFieldNameById(row.tag))">
                  <v-combobox dense outlined v-model="row.value" label="Value" class="flex-grow-1"
                    :items="getDynamicSuggestions(row.tag)" item-text="mproduct_type_name" item-value="mproduct_type_id"
                    :return-object="true" />
                </div>
                <div v-else-if="['Brand'].includes(getFieldNameById(row.tag))">
                  <v-combobox dense outlined v-model="row.value" label="Value" class="flex-grow-1"
                    :items="getDynamicSuggestions(row.tag)" item-text="mbrand_name" item-value="mbrand_id"
                    :return-object="true" />
                </div>
                <div v-else>
                  <v-combobox dense outlined v-model="row.value" label="Value" class="flex-grow-1"
                    :items="getDynamicSuggestions(row.tag)" item-text="mtag_name" item-value="mtag_id"
                    :return-object="true" />
                </div>

                <v-btn class="btn-32-text-12" icon v-if="conditions.length > 1" @click="removeCondition(idx)">
            ...
            <v-btn style="color: #1976d2; background-color: white !important; border: 1px solid #1976d2 !important;"
              class="btn-32-text-12" outlined small @click="addCondition"><v-icon small>mdi-plus</v-icon> Add another
              condition</v-btn>
          </v-card-text>
        </v-card>
      </v-col>
      ...
      <v-col cols="12" md="3">
        <v-card elevation="5" class="mb-3">
          ...
          <v-card-text>
            <v-row>
              <v-col cols="12" md="12">
                <div class="text-subtitle-2 font-weight-medium mb-2 fw-bold">Sales Channels</div>

                <div class="d-flex align-center" style="margin-top: 8px;">
                  <v-checkbox v-model="msubcat_publish" value="Online Store" hide-details class="pa-0" style="margin: 0; 
                            padding-right: 4px;" density="compact"></v-checkbox>
                  <span style="font-size: 13px; margin-top: 1px;">Online Store</span>
                </div>

                <div class="d-flex align-center" style="margin-top: 4px;">
                  <v-checkbox v-model="msubcat_publish" value="Other" hide-details class="pa-0"
                    style="margin: 0; padding-right: 4px;" density="compact"></v-checkbox>
                  <span style="font-size: 13px; margin-top: 1px;">Other</span>
                </div>
              </v-col>
            </v-row>
          </v-card-text>
        ...
        <v-card elevation="5">
          ...
          <v-card-text>
            ...
            <div v-else class="d-flex align-center">
              ...
              <v-btn class="btn-32-text-12 ml-2" icon small @click="clearImage"><v-icon
        ...
        <v-card elevation="5" class="mt-3">
          ...
          <v-text-field class="px-4 mt-3" v-model="form.start_time" label="Start Time" type="datetime-local" dense
            outlined :rules="startTimeRules" />
          <v-text-field class="px-4 mt-3" v-model="form.end_time" label="End Time" type="datetime-local" dense outlined
            :rules="endTimeRules" />
        </v-card>
    ...
    <v-dialog v-model="productDialog" max-width="650">
      <v-card elevation="5">
        ...
        <v-data-table v-model="productSelection" :items="allProducts" :headers="productHeaders" :search="productSearch"
        ...
        <v-card-actions>
          ...
          <v-btn class="btn-32-text-12"
            ...
            @click="confirmProducts">Done</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
.page-sub-cat-add .v-select.v-input input {
  height: 24px !important;
