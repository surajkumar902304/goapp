...
export default {
  ...
  computed:{
    msubcatnameRule(){
      return [
        ...
        v=>!this.msubcats.some(s =>
             s.msubcat_name===v && s.msubcat_id!==this.msubcatid)
    ...
    offerNameRules(){
      return [
        v=>(v&&v.length>=3) || 'Name must be at least 3 characters',
        v=>!this.msubcats.some(s =>
             s.offer_name===v && s.msubcat_id!==this.msubcatid)
           || 'Sub-Category already exists'
      ]
    },
  ...
  /* ---------- life‑cycle ---------- */
  created(){
    /* पहले सारी look‑up lists ले आओ, फिर existing data बिठाओ */
    Promise
  ...
  methods:{
    ...
    resetSubcat(){ this.form.subcatname=''; this.nameError='' },
    checkDuplicate(){
      if(!this.form.mcat_id||!this.form.subcatname.trim()) return
      this.nameError = this.msubcats.some(s =>
